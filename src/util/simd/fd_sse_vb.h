...
/* vb_ld return the 16 uchars at the 16-byte aligned / 16-byte sized
   ...
   16-byte sized location p as 32 uchars.  vb_stu is the same but p does not
...
// /* vb_ldif is an optimized equivalent to vb_notczero(c,vb_ldu(p)) (may
//    have different behavior if c is not a proper vector conditional).  It
//    is provided for symmetry vbth the vb_stif operation.  vb_stif stores
//    x(n) to p[n] if c(n) is true and leaves p[n] unchanged othervbse.
//    Undefined behavior if c is not a proper vector conditional. */
// /* Sadly, no maskload_epi8 */
//
// #define vb_ldif(c,p)   _mm_maskload_epi32( (p),(c))
// #define vb_stif(c,p,x) _mm_maskstore_epi32((p),(c),(x))
//
...
/* Not easy given the intrinsics. Could convert to 16b, so the shift,
   and convert back, but that's terrible. */
//#define vb_shr(a,imm)
...
//#define vb_shr_variable(a,n)  _mm_sra_epi32( (a), _mm_insert_epb64( _mm_setzero_sb128(), (n), 0 ) )
#define vb_shru_variable(a,n) vb_and( _mm_srl_epi16( (a), _mm_insert_epi64( _mm_setzero_si128(), (n), 0 ) ), \
                                      vb_bcast( (uchar)(0xFFUL >> (n)) ) )

//#define vb_shl_vector(a,b)
//#define vb_shr_vector(a,b)
//#define vb_shru_vector(a,b)
...
#define vb_lnotnot(a)                                            /* [ !!a0 !!a1 ... !!a15 ] */ \
  _mm_xor_si128( _mm_set1_epi32( -1 ), _mm_cmpeq_epi8( (a), _mm_setzero_si128() ) )
...
//#define vb_to_wc_raw(a) (a)
//#define vb_to_wf_raw(a) _mm_castsi128_ps( (a) )
//#define vb_to_wd_raw(a) _mm_castsi128_pd( (a) )
//#define vb_to_wl_raw(a) (a)
