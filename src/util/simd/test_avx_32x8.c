...
# define brand() (uchar)((fd_rng_uchar( rng ) % 7U)-3U)                /* [253,254,255,0,1,2,3] */
  ...
  for( int i=0; i<65536; i++ ) {
    ...
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = yi[0];            FD_TEST( wb_test( wb_bcast( yi[0] ), ti ) );

    for( ulong j=0UL; j<32UL; j++ ) ti[j] = yi[j&1UL];        FD_TEST( wb_test( wb_bcast_pair( yi[0], yi[1] ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = yi[j>>4 ];        FD_TEST( wb_test( wb_bcast_lohi( yi[0], yi[1] ), ti ) );

    for( ulong j=0UL; j<32UL; j++ ) ti[j] = yi[j&3UL];        FD_TEST( wb_test( wb_bcast_quad( yi[0], yi[1], yi[2], yi[3] ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = yi[j>>1 ];        FD_TEST( wb_test( wb_bcast_wide( EXPAND_16_INDICES( yi, 0 ) ), ti ) );

    for( ulong j=0UL; j<32UL; j++ ) ti[j] = yi[j & (~1UL)];   FD_TEST( wb_test( wb_bcast_even(     y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = yi[j |   1   ];   FD_TEST( wb_test( wb_bcast_odd(      y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = yi[j ^   1   ];   FD_TEST( wb_test( wb_exch_adj(       y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = yi[j ^   2   ];   FD_TEST( wb_test( wb_exch_adj_pair(  y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = yi[j ^   4   ];   FD_TEST( wb_test( wb_exch_adj_quad(  y ), ti ) );

    /* Bit operations */

    for( ulong j=0UL; j<32UL; j++ ) ti[j] = (uchar)~yi[j];    FD_TEST( wb_test( wb_not( y ), ti ) );
...
#   define _(n)                                                                                           \
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = (uchar)(yi[j]<<n); FD_TEST( wb_test( wb_shl ( y, n ), ti ) ); \
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = (uchar)(yi[j]>>n); FD_TEST( wb_test( wb_shru( y, n ), ti ) )
    _( 0); _( 1); _( 2); _( 3); _( 4); _( 5); _( 6); _( 7); _( 8); _( 9); _(10); _(11); _(12); _(13); _(14); _(15); _(16);
#   undef _

    for( int n=0; n<8; n++ ) {
      int volatile m[1]; m[0] = n;
      for( ulong j=0UL; j<32UL; j++ ) ti[j] = (uchar)(yi[j]<<n);     FD_TEST( wb_test( wb_shl_variable(  y, m[0] ), ti ) );
      for( ulong j=0UL; j<32UL; j++ ) ti[j] = (uchar)(yi[j]>>n);     FD_TEST( wb_test( wb_shru_variable( y, m[0] ), ti ) );
    }

#   undef ROR
#   undef ROL

    for( ulong j=0UL; j<32UL; j++ ) ti[j] =   xi[j]  & yi[j];        FD_TEST( wb_test( wb_and(    x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = ((uchar)~xi[j]) & yi[j]; FD_TEST( wb_test( wb_andnot( x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] =   xi[j]  | yi[j];        FD_TEST( wb_test( wb_or(     x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] =   xi[j]  ^ yi[j];        FD_TEST( wb_test( wb_xor(    x, y ), ti ) );

    /* Arithmetic operations */

    for( ulong j=0UL; j<32UL; j++ ) ti[j] = (uchar)-xi[j];                FD_TEST( wb_test( wb_neg( x    ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = fd_uchar_abs( xi[j] );        FD_TEST( wb_test( wb_abs( x    ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = fd_uchar_min( xi[j], yi[j] ); FD_TEST( wb_test( wb_min( x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = fd_uchar_max( xi[j], yi[j] ); FD_TEST( wb_test( wb_max( x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = (uchar)(xi[j]+yi[j]);         FD_TEST( wb_test( wb_add( x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = (uchar)(xi[j]-yi[j]);         FD_TEST( wb_test( wb_sub( x, y ), ti ) );

    /* Logical operations */

#define C(cond) (uchar)( (cond) ? 0xFF : 0 )
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = C( !xi[j]);        FD_TEST( wb_test( wb_lnot(    x ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = C(!!xi[j]);        FD_TEST( wb_test( wb_lnotnot( x ), ti ) );

    for( ulong j=0UL; j<32UL; j++ ) ti[j] = C(xi[j]==yi[j]);   FD_TEST( wb_test( wb_eq( x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = C(xi[j]> yi[j]);   FD_TEST( wb_test( wb_gt( x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = C(xi[j]< yi[j]);   FD_TEST( wb_test( wb_lt( x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = C(xi[j]!=yi[j]);   FD_TEST( wb_test( wb_ne( x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = C(xi[j]>=yi[j]);   FD_TEST( wb_test( wb_ge( x, y ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = C(xi[j]<=yi[j]);   FD_TEST( wb_test( wb_le( x, y ), ti ) );

    for( ulong j=0UL; j<32UL; j++ ) ti[j] = (uchar)(ci[j]?0:xi[j]);       FD_TEST( wb_test( wb_czero(    c, x ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] = (uchar)(ci[j]?xi[j]:0);       FD_TEST( wb_test( wb_notczero( c, x ), ti ) );
    for( ulong j=0UL; j<32UL; j++ ) ti[j] =        (ci[j]?xi[j]:yi[j]);   FD_TEST( wb_test( wb_if( c, x, y ),    ti ) );
#undef C
    ...
    ulong acc;
    acc=0UL;   for( ulong j=0UL; j<32UL; j++ ) acc += xi[j];  FD_TEST( !wb_any( wb_ne( wb_sum_all( x ), wb_bcast( (uchar)acc ) )));
