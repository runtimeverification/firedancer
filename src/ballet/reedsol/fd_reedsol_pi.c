...
static inline ws_t ws_ldu( short const * p   ) { return _mm256_loadu_si256( (__m256i const *)p ); }
static inline void ws_st(  short * p, ws_t i ) { _mm256_store_si256(  (__m256i *)p, i ); }
static inline void ws_stu( short * p, ws_t i ) { _mm256_storeu_si256( (__m256i *)p, i ); }
...
static inline void print_ws( char const * name, ws_t v ) {
  short buf[ 16 ] W_ATTR;
  ws_st( buf, v );
  printf("%-10s: ", name );
  for( int i=0; i<16; i++ ) printf("% 6hi", buf[ i ]);
  printf("\n");
}
static inline void print_wb( char const * name, wb_t v ) {
  uchar buf[ 32 ] W_ATTR;
  wb_st( buf, v );
  printf("%-10s: ", name );
  for( int i=0; i<32; i++ ) printf("%4u", (uint)buf[ i ]);
  printf("\n");
}
